...
function App() {
  ...
  return <div className="app-shell">
    ...
    <div className="app-content"><header className="topbar"><p><span className="live-dot" />Eligibility intelligence</p><span>Built for informed choices</span></header><main>
      {activeTab === 'dashboard' && <><section className="hero-grid"><div className="hero-copy"><p className="eyebrow">A clearer financial path</p><h1>Support that<br /><i>fits your life.</i></h1><p className="lede">ADITI turns public schemes and financial decisions into practical next steps — shaped around your situation.</p><button className="signal-button" onClick={() => setActiveTab('match')}>Find my schemes <span>→</span></button></div><div className="signal-field" aria-label="Eligibility match preview"><div className="signal-orbit orbit-one" /><div className="signal-orbit orbit-two" /><div className="signal-core"><small>DISCOVER</small><strong>{schemesCount || '—'}</strong><span>available schemes</span></div><p>One profile<br />many possibilities</p></div></section><section className="dashboard-grid"><div className="workspace"><div className="section-heading"><span>Start with your profile</span><small>About two minutes</small></div><MatchMaker onSubmit={handleMatch} loading={loading} compact /></div><div className="results-pane">{error && <p className="inline-error">{error}</p>}{results}</div></section><Subscription /></>}
