...
export default function Catalogue() {
  ...
  useEffect(() => { axios.get('/api/schemes').then(({ data }) => setSchemes(data.schemes || [])).catch(() => setError('The scheme catalogue is unavailable right now.')).finally(() => setLoading(false)); }, []);
  const openScheme = async (id) => { try { const { data } = await axios.get(`/api/schemes/${id}`); setSelected(data); } catch { setError('We could not load that scheme.'); } };
